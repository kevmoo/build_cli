...
- Required `analyzer: ">=10.0.0 <15.0.0"`
...
- Change the name of the private `Map` used to support enums so that it does not
  conflict with the one generated by `package:json_serializable`.
...
- Generate all local variables as `final` where applicable. Allows consumers to
  enable the `prefer_final_locals` lint.
...
- Generate another private method `_$populate[OptionClassName]Parser`. Allows
  usage for already existing `ArgParser` instances, such as with `package:args`
  `CommandRunner`.
